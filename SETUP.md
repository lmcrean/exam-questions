...
2. Add the following secrets:
   ...
   |-------------|-------------|-----------------|
   | `GCP_SA_KEY` | GCP service account JSON | GCP Console → IAM → Service Accounts |
   | `GCP_PROJECT_ID` | Your GCP project ID | Same as in .env |
   | `FIREBASE_SERVICE_ACCOUNT_KEY` | Firebase service account JSON | Firebase Console → Project Settings |
...
- ✅ `apps/web/.firebaserc` - Web Firebase project
- ✅ `apps/web/public/favicon/site.webmanifest` - PWA manifest
- ✅ `.github/project-config.yml` - CI/CD config

### Use Environment Variables (No Manual Changes Needed)
- ✅ `apps/api/server.ts` - CORS, startup messages
- ✅ `apps/api/routes/setup/health/hello.ts` - API greeting
- ✅ `apps/api/services/webhookService.ts` - User-Agent header
- ✅ `apps/api/test-utilities/urls.ts` - Test URLs
- ✅ `apps/api/test-prod-api.ts` - Production test URLs
- ✅ `apps/api/test-vertex-ai.ts` - GCP project ID
- ✅ `apps/api/playwright.prod.config.ts` - Playwright base URL
- ✅ `gcp-setup/deploy-api.sh` - Deployment script
- ✅ `gcp-setup/deploy-web.sh` - Deployment script
- ✅ `gcp-setup/setup.sh` - GCP setup script
- ✅ `e2e/playwright.*.ts` - E2E test configs
- ✅ `apps/web/index.html` - Page title (uses Vite %VITE_APP_DISPLAY_NAME%)

### Optional Manual Updates (For Complete Rebrand)
- 📝 `apps/web/package.json` - Package name
- 📝 All documentation files in `docs/`
- 📝 README.md files
- 📝 Test files that check for specific text (e.g., "Hello World from Dottie API!")
...
1. **Test environment loading**
   ```bash
   npm run dev
   # Should show: "🚀 Starting [Your App Name] server..."
   ```
...
3. **Test web app**
   ...
   - Check PWA manifest (View Page Source → find manifest link)
...
1. **Custom Cloud Run regions**
   ...
   - Update `.github/project-config.yml` → `cloud_run.region`
